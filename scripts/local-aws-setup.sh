...
until (curl http://localhost:8080 | grep "\"s3\": \"\(running\|available\)\"" > /dev/null); do
    sleep 5
done
...
aws --endpoint-url=http://localhost:8080 s3api create-bucket --bucket fragments
...
# [URL documentation]
aws --endpoint-url=http://localhost:8080 dynamodb wait table-exists --table-name fragments
